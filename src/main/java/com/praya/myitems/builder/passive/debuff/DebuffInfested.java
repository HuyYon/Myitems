...
public class DebuffInfested extends PassiveEffect
{
    ...
    public DebuffInfested() {
        super(DebuffInfested.debuff, 1);
    }

    public DebuffInfested(final int grade) {
        super(DebuffInfested.debuff, grade);
    }
    ...
    public final void cast(final Player player) {
        ...
        if (!passiveEffectManager.isPassiveEffectCooldown(DebuffInfested.debuff, (OfflinePlayer)player)) {
            ...
            passiveEffectManager.setPassiveEffectCooldown(DebuffInfested.debuff, (OfflinePlayer)player, cooldown);
        }
    }
    
    private final long getCooldown() {
        final MainConfig mainConfig = MainConfig.getInstance();
        return (long)(MathUtil.convertTickToMilis(mainConfig.getPassivePeriodEffect()) * DebuffInfested.debuff.getMaxGrade() / 2.5);
    }
}
