...
public class DebuffDarkness extends PassiveEffect
{
    ...
    public DebuffDarkness() {
        super(DebuffDarkness.debuff, 1);
    }

    public DebuffDarkness(final int grade) {
        super(DebuffDarkness.debuff, grade);
    }
    ...
    public final void cast(final Player player) {
        ...
        if (!passiveEffectManager.isPassiveEffectCooldown(DebuffDarkness.debuff, (OfflinePlayer)player)) {
            ...
            passiveEffectManager.setPassiveEffectCooldown(DebuffDarkness.debuff, (OfflinePlayer)player, cooldown);
        }
    }
    
    private final long getCooldown() {
        final MainConfig mainConfig = MainConfig.getInstance();
        return (long)(MathUtil.convertTickToMilis(mainConfig.getPassivePeriodEffect()) * DebuffDarkness.debuff.getMaxGrade() / 2.5);
    }
}
