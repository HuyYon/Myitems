...
public class SocketManager extends HandlerManager {
    ...
    public final boolean containsSocketEmpty(final ItemStack item) {
        return !this.getLineLoresSocketEmpty(item).isEmpty();
    }
    ...
    public final boolean containsSocketLocked(final ItemStack item) {
        return !this.getLineLoresSocketLocked(item).isEmpty();
    }
    ...
    public final boolean containsSocketGems(final ItemStack item) {
        return !this.getLineLoresSocket(item).isEmpty();
    }
    ...
    public final SocketGemsProperties getSocketProperties(final LivingEntity livingEntity, final boolean checkDurability) {
        ...
        // 1.21.4: Slot.values() now includes BODY (used by wolves/horses armour).
        // Bridge.getBridgeEquipment().getEquipment() should handle unsupported slots
        // gracefully by returning null; EquipmentUtil.isSolid() guards against that.
        for (final Slot slot : Slot.values()) {
            final ItemStack item = Bridge.getBridgeEquipment().getEquipment(livingEntity, slot);
            if (item != null && EquipmentUtil.isSolid(item)) {
        ...
        return new SocketGemsProperties(
                additionalDamage, percentDamage, penetration, pvpDamage, pveDamage,
                additionalDefense, percentDefense, health, healthRegen, staminaMax, staminaRegen,
                attackAoERadius, attackAoEDamage, pvpDefense, pveDefense,
                criticalChance, criticalDamage, blockAmount, blockRate, hitRate, dodgeRate);
    }
